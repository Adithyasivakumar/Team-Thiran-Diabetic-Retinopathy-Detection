...
class PDFReportGenerator:
    ...
    def generate_pdf(self, report_data, image_path=None):
        ...
        patient_table_data = [
            [
                f"<b>Patient Name:</b> {patient_data['name']}",
                f"<b>Report ID:</b> {report_data['report_id']}"
            ],
            [
                f"<b>Patient ID:</b> {patient_data['id']}",
                f"<b>Report Date:</b> {report_data['report_date']}"
            ],
            [
                f"<b>Age:</b> {patient_data['age']} years",
                f"<b>Physician:</b> {screening_data['physician']}"
            ],
            [
                f"<b>Gender:</b> {patient_data['gender']}",
                f"<b>Modality:</b> {screening_data['imaging_modality']}"
            ]
        ]
        ...
        findings_table_data = [
            [
                ...
                f"<font color='{risk_color}'><b>{findings['classification']}</b></font>"
            ],
            [
                f"<b>Risk Level:</b>",
                f"<font color='{risk_color}'><b>{findings['risk_level']}</b></font>"
            ],
            [
                f"<b>Model Confidence:</b>",
                f"{findings['confidence']}"
            ],
            [
                f"<b>Image Analyzed:</b>",
                f"{findings['image_analyzed']}"
            ]
        ]
        ...
        story.append(Spacer(1, 0.1*inch))
        recommendation_text = f"""<b>Recommendation:</b> {report_data['recommendations']}<br/><br/>"""
        ...
        for step in report_data['next_steps']:
            steps_text += f"<li>{step}</li>"
        steps_text += "</ol>"
        story.append(Paragraph(steps_text, assessment_style))
        story.append(Spacer(1, 0.2*inch))
        ...
        This report is generated by an AI-assisted screening system and should be reviewed and confirmed by a qualified ophthalmologist. 
        The AI model has been trained on medical datasets but is not a substitute for professional medical judgment. 
